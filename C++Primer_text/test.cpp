...
// 练习4.20 P134
//(a):合法。先对iter + 1，在返回iter指向的值。
//(b):不合法。返回iter指向的值是string，++操作无意义。
//(c):不合法。iter是一个指针，没有empty成员。
//(d):合法。等价于(*iter).empty()。判断iter指向的值是否为空。
//(e):不合法。 * iter可以得到iter所指的字符串，但是字符串并没有++操作。
//(f): 首先判断iter所指向的值是否为空，再对iter加1。
...
//~'q' << 6
//1111 1111 1111 1111 1110 0011 1000 0000   值为： - 7296
